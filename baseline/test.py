...
parser.add_argument('--divide', default=4, type=int, help='divide the target image to get better resolution') #分割目标图片
args = parser.parse_args()

canvas_cnt = args.divide * args.divide
T = torch.ones([1, 1, width, width], dtype=torch.float32).to(device)
img = cv2.imread(args.img, cv2.IMREAD_COLOR) #读入图片
...
Decoder = FCN()
Decoder.load_state_dict(torch.load(args.renderer)) #加载训练好的渲染器
...
def decode(x, canvas): # b * (10 + 3)
    ...
    color_stroke = color_stroke.permute(0, 3, 1, 2) #维度换位
...
#num_inputs,depth,num_output      depth=18：BasicBlock, [2,2,2,2]
